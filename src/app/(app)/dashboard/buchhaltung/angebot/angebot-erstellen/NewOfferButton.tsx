// src/app/(app)/dashboard/buchhaltung/angebot/angebot-erstellen/NewOfferButton.tsx
'use client'
...
export default function NewOfferButton() {
  ...
  const handleClick = async () => {
    ...
    setError(null)
    try {
      const res = await fetch('/api/angebot/next-offer-number')
      if (!res.ok) throw new Error(await res.text())
      const { nextNumber } = await res.json()
      router.push(
        `/dashboard/buchhaltung/angebot/angebot-erstellen?offerNumber=${encodeURIComponent(nextNumber)}`
      )
    } catch (e: any) {
      console.error(e)
