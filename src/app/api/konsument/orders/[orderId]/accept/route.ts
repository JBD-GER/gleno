// src/app/api/konsument/orders/[orderId]/accept/route.ts
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ orderId: string }> }) {
  try {
    ...
    if (order.status === 'accepted') {
      await tryUpdateRequestStatus(supabase, order.request_id, ['Auftrag angenommen'])
    ...
    // Status → accepted
    {
      const { error } = await supabase.from('market_orders').update({ status: 'accepted' }).eq('id', order.id)
      if (error) return NextResponse.json({ ok:false, error: error.message }, { status:500 })
    }

    await tryUpdateRequestStatus(supabase, order.request_id, ['Auftrag angenommen'])

    const plain = `ORDER:ACCEPTED:${order.id} • Auftrag angenommen: ${order.title ?? ''} • Brutto: ${Number(order.gross_total||0).toFixed(2)} €`
    const html  = `
      <div style="font-family:ui-sans-serif,system-ui;-webkit-font-smoothing:antialiased">
        <div style="border:1px solid #d1fae5;background:#ecfdf5;border-radius:14px;padding:12px">
          <div style="font-size:14px;font-weight:600;color:#065f46;margin-bottom:4px">Auftrag angenommen</div>
          <div style="font-size:12px;color:#065f46">${escapeHtml(order.title ?? '')}</div>
          <div style="font-size:12px;color:#065f46;margin-top:6px">Brutto: ${Number(order.gross_total||0).toFixed(2)} €</div>
        </div>
      </div>
    `.trim()
