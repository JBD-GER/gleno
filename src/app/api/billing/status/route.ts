...
export async function GET() {
  try {
    ...
    if (p.stripe_subscription_id) {
      try {
        // wichtiger harter Timeout
        const s: any = await withTimeout(
          stripe.subscriptions.retrieve(p.stripe_subscription_id, {
        ...
        // Best-effort Self-Healing – Fehler ignorieren
        const patch: Record<string, any> = {}
        if (!p.current_period_end && sub_period_end_iso_live)
          patch.current_period_end = sub_period_end_iso_live
        if (cancel_at_period_end && p.subscription_status === 'active')
        ...
        // Stripe down / Timeout → wir antworten mit DB-Infos weiter unten.
      }
    }
    ...
    if (
      (activeOk && cancel_at_period_end && current_period_end_iso) ||
      (graceOk && current_period_end_iso)
    ) {
