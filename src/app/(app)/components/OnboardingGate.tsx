...
function Pill({ ok, label }: { ok: boolean; label: string }) {
  return (
    ...
    >
      {ok ? <CheckCircleIcon className="h-4 w-4" /> : <ExclamationTriangleIcon className="h-4 w-4" />}
      {label}
    </span>
  )
}
...
type UsersApiProfile = {
  profile?: {
    ...
    vat_number?: string | null               // <-- hinzugefügt
  } | null
...
export default function OnboardingGate() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const needNumbering =
          s?.quote_prefix == null || s?.quote_start == null || s?.quote_suffix == null ||
          s?.invoice_prefix == null || s?.invoice_start == null || s?.invoice_suffix == null
  ...
  const status = useMemo(() => {
    const s = settings
    const billingDone = !!(s?.account_holder && s?.iban && s?.bic && s?.billing_phone && s?.billing_email)
    const numberingDone = !(
      s?.quote_prefix == null || s?.quote_start == null || s?.quote_suffix == null ||
      s?.invoice_prefix == null || s?.invoice_start == null || s?.invoice_suffix == null
    )
  ...
  const canSaveStep2 =
    !!form &&
    !!form.account_holder && !!form.iban && !!form.bic && !!form.billing_phone && !!form.billing_email &&
    form.quote_prefix !== undefined && form.quote_start !== undefined && form.quote_suffix !== undefined &&
    form.invoice_prefix !== undefined && form.invoice_start !== undefined && form.invoice_suffix !== undefined &&
    ibanOk && bicOk
  ...
  const saveStep2 = async () => {
    ...
    setSaving(true)
    try {
      // NEU: USt-IdNr. im Profil speichern (optional)
      ...
      if (!res.ok) {
        const { message } = await res.json().catch(() => ({ message: 'Fehler beim Speichern' }))
        alert(message); return
      }
      const refreshed = await (await fetch('/api/billing-settings')).json()
      setSettings(refreshed); setForm(refreshed); setStep(2)
    } finally {
      setSaving(false)
    }
  }
  ...
  const saveStep3 = async () => {
    ...
    try {
      ...
      const refreshed = await (await fetch('/api/billing-settings')).json()
      setSettings(refreshed); setStep(3); router.refresh()
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleNewFile = (f: File) => {
    setLogoError(null); setLogoSuccess(null)
    const okType = ['image/png','image/jpeg'].includes(f.type)
    const okSize = f.size <= 5 * 1024 * 1024 // 5MB
    if (!okType) { setLogoError('Nur PNG oder JPG erlaubt.'); return }
    if (!okSize) { setLogoError('Maximale Dateigröße: 5 MB.'); return }
    setFile(f)
    uploadLogo(f).catch(() => {})
  }
  ...
  const uploadLogo = async (directFile?: File) => {
    ...
    if (!f) return
    setUploadingLogo(true); setLogoError(null); setLogoSuccess(null)
    try {
      ...
      if (!res.ok) {
        ...
        setLogoError(message || 'Upload fehlgeschlagen'); return
      }
    ...
    <div className="fixed inset-0 z-[1200]">
      ...
      <div
        className={clsx(
          ...
          'md:left-1/2 md:top-1/2 md:h-auto md:w-[min(1100px,95vw)] md:-translate-x-1/2 md:-translate-y-1/2 md:rounded-2xl',
          'overflow-hidden border border-white/60 bg-white/92 shadow-[0_25px_90px_rgba(2,6,23,0.35)] backdrop-blur-xl'
        )}
      >
        {/* Header (sticky) */}
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-white/60 bg-white/90 px-4 py-3 md:px-5 md:py-4 backdrop-blur">
          <div className="flex min-w-0 items-center gap-2">
            <h1 className="truncate text-base font-semibold text-slate-900">Erstes Setup</h1>
          ...
          <div className="sm:hidden text-[11px] text-slate-600">Schritt {step + 1}/4</div>
        </div>

        {/* Body (scrollbar) */}
        <div
          className="grid h-[calc(100svh-48px-56px)] md:h-auto md:max-h-[72vh] grid-rows-[1fr] md:grid-rows-[auto_1fr_auto]"
        >
          {/* Inhalt-Scroller mobil; auf Desktop übernimmt die innere Grid-Zeile */}
          <div
            className="overflow-y-auto px-4 py-4 md:px-5 md:py-4"
            ...
            {step === 1 && !!form && (
              <>
                <StepHeader
                  ...
                  subtitle="Diese Angaben erscheinen auf Angeboten/Rechnungen."
                />

                {/* Hinweis zur USt-IdNr. und E-Rechnung */}
                <div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">
                  <strong>Hinweis:</strong> Die USt-IdNr. ist für die Erstellung einer E-Rechnung (EN&nbsp;16931/XRechnung) erforderlich.
                  In Deutschland wird die E-Rechnung im B2B-Bereich stufenweise verpflichtend (Start 2025; Ausstellungspflichten je nach Unternehmensgröße ab 2027/2028).
                </div>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    ...
                    {!ibanOk && <p className="text-xs text-rose-600">Ungültige IBAN.</p>}
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    ...
                    {!bicOk && <p className="text-xs text-rose-600">Ungültige BIC.</p>}
                  </div>
                ...
                <div className="mt-5 rounded-xl border border-white/60 bg-white/80 p-4">
                  <div className="mb-2 text-sm font-semibold text-slate-800">Nummernkreise</div>
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                    ...
                    <Field label="Angebot Start">
                      <NumberInput
                        ...
                        onChange={(e) => setForm({ ...form, quote_start: Number(e.target.value || 1) })}
                      />
                    </Field>
                    <Field label="Angebot Suffix">
                      <TextInput
                        value={form.quote_suffix ?? ''}
                        onChange={(e) => setForm({ ...form, quote_suffix: e.target.value })}
                      />
                    </Field>
                    ...
                    <Field label="Rechnung Start">
                      <NumberInput
                        ...
                        onChange={(e) => setForm({ ...form, invoice_start: Number(e.target.value || 1) })}
                      />
                    </Field>
                    <Field label="Rechnung Suffix">
                      <TextInput
                        value={form.invoice_suffix ?? ''}
                        onChange={(e) => setForm({ ...form, invoice_suffix: e.target.value })}
                      />
                    </Field>
                  </div>
                </div>
              ...
              <>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-12">
                  <div className="md:col-span-7">
                    <div className="h-[380px] md:h-[420px] overflow-hidden rounded-xl border border-white/60 bg-white/80">
                      <TemplatePreview name={selectedTemplate || settings?.template || ''} />
                    </div>
                  </div>
                  <div className="md:col-span-5">
                    <div
                      className="max-h-[380px] md:max-h-[420px] overflow-auto rounded-xl border border-white/60 bg-white/80 p-2"
                      style={{ WebkitOverflowScrolling: 'touch' }}
                    >
                      {files.length === 0 ? (
                        <div className="p-3 text-sm text-slate-600">Keine Vorlagen im Bucket gefunden.</div>
                      ) : (
                        <div className="grid grid-cols-1 gap-2">
                          {files.map((name) => (
                            <label
                              key={name}
                              className={clsx(
                                'flex cursor-pointer items-center gap-2 rounded-lg border px-2 py-2 text-sm shadow-sm',
                            ...
                            >
                              <input
                                ...
                                name="tpl"
                                value={name}
                                checked={(selectedTemplate || settings?.template) === name}
                  ...
                  <div>
                    ...
                    >
                      ...
                      <p className="mt-2 text-xs text-slate-500">
                        Hinweis: PNG oder JPG, ideal 1500×600&nbsp;px (mind. 1250×500&nbsp;px), sRGB.
                        PNG mit transparentem Hintergrund empfohlen; SVG/WebP werden nicht unterstützt.
                      </p>
                      {uploadingLogo && (
          ...
          <div className="sticky bottom-0 border-t border-white/60 bg-white/90 px-4 py-3 md:px-5 md:py-3 backdrop-blur">
            <div className="flex items-center justify-between">
              <div className="hidden sm:block text-xs text-slate-600">Schritt {step + 1} / 4</div>
...
function TemplatePreview({ name }: { name: string }) {
  ...
  useEffect(() => {
    let ignore = false
    ;(async () => {
      if (!name) { if (!ignore) setUrl(null); return }
      try {
        const { data } = supabase.storage.from('rechnungvorlagen').getPublicUrl(name)
        if (!ignore) setUrl(data.publicUrl ?? null)
      } catch {
