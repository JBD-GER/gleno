...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Admin-Client umgeht RLS, filtert aber sauber auf companyId
    ...
    // einfache Serversuche
    if (search) {
      query = query.ilike('first_name', `%${search}%`)
                   .ilike('last_name', `%${search}%`) // Hinweis: Supabase matcht OR nicht automatisch
    }

    // Besser: OR für first_name/last_name/email
    if (search) {
      query = supabaseAdmin
        .from('customers')
        .select('id, first_name, last_name, email')
