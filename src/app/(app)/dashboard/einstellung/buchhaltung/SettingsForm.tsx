// src/app/(app)/dashboard/einstellung/buchhaltung/SettingsForm.tsx
'use client'
...
export default function SettingsForm({ initial }: { initial: BillingSettings }) {
  ...
  const inputCls =
    'w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-800 shadow-sm outline-none ring-indigo-100/60 transition focus:ring-2 focus:ring-indigo-200'
    ...
    <div className="space-y-8">
      ...
      <section className="rounded-xl border border-slate-100 bg-slate-50/60 p-3 sm:p-4">
        ...
        <p className="mb-4 text-xs text-slate-500">
          Prefix/Suffix und Startnummern für deine Dokument-Nummern.
        </p>
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          {[
            ...
            {
              label: 'Startnummer Angebot',
              name: 'quote_start',
              type: 'number',
            },
            {
              label: 'Auftragsbestätigung Prefix',
              name: 'order_confirmation_prefix',
              type: 'text',
            },
            {
              label: 'Auftragsbestätigung Suffix',
              name: 'order_confirmation_suffix',
              type: 'text',
            },
            {
              label: 'Startnummer Auftragsbestätigung',
              name: 'order_confirmation_start',
              type: 'number',
            },
          ].map((f) => (
            <div key={f.name}>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          {[
            ...
            <div key={f.name}>
              ...
              />
              {f.name === 'iban' && !ibanOk && (
                <p className="mt-1 text-xs text-rose-600">Ungültige IBAN.</p>
              )}
              {f.name === 'bic' && !bicOk && (
                <p className="mt-1 text-xs text-rose-600">Ungültige BIC.</p>
              )}
            </div>
          ))}
        </div>
      ...
      <section className="rounded-xl border border-slate-100 bg-slate-50/60 p-3 sm:p-4">
        ...
        <p className="mb-4 text-xs text-slate-500">
          Verlinke hier deine AGB und Datenschutzerklärung.
        </p>
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          {[
      ...
      <div className="flex justify-end">
        <button
          ...
          className="inline-flex items-center justify-center rounded-lg bg-slate-900 px-6 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-black disabled:cursor-not-allowed disabled:opacity-50"
        >
