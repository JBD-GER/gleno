...
function absoluteUrl(href: string, origin: string) {
  try { const u = new URL(href, origin); if (u.origin !== origin && !href.startsWith('/')) return null; return u.href } catch { return null }
}
...
async function fetchText(url: string) {
  ...
  try {
    const res = await fetch(url, { redirect: 'follow', headers: { 'User-Agent': 'PartnerBot/3.0' }, signal: ctrl.signal })
    return await res.text()
  } catch { return '' } finally { clearTimeout(to) }
}
function extractInternalLinks(html: string, base: string) {
  const hrefs = Array.from(html.matchAll(/<a[^>]+href\s*=\s*["']([^"']+)["']/gi)).map(m => m[1])
  const cleaned = hrefs
    .map(h => h.trim())
  ...
  const scored = cleaned.map(u => {
    ...
    const score = PAGE_KEYWORDS.reduce((acc, kw) => acc + (path.includes(kw) ? 1 : 0), 0)
    return { u, score }
  })
...
function discoverSocials(html: string) {
  ...
  const add = (re: RegExp, kind: string) => { const m = html.match(re); if (m?.[0]) out.push({ url: m[0], kind }) }
...
function pickMeta(html: string) {
  ...
  const title = html.match(/<title[^>]*>([^<]+)<\/title>/i)?.[1]; if (title) out.title = title.trim()
  const meta = (name: string, attr='name') =>
    html.match(new RegExp(`<meta[^>]+${attr}\\s*=\\s*["']${name}["'][^>]+content\\s*=\\s*["']([^"']+)["'][^>]*>`, 'i'))?.[1]
  out.description = meta('description') || meta('og:description','property') || ''
  const ldjsons = Array.from(html.matchAll(/<script[^>]+type=["']application\/ld\+json["'][^>]*>([\s\S]*?)<\/script>/gi)).map(m => m[1])
...
export async function POST(req: NextRequest) {
  try {
    ...
    const DB_BRANCHES: { id:string, name:string, slug:string }[] = (branchRows || [])
      .map(b => ({ id: String(b.id), name: String(b.name), slug: String(b.slug) }))

    // Crawling
    const seeds: string[] = []
    for (const raw of links || []) {
      try { const u = new URL(raw.startsWith('http') ? raw : `https://${raw}`); seeds.push(u.href) } catch {}
    }
    const seed = seeds[0]
    ...
    const combined = [freeText, ...pages.map(p => p.text)].filter(Boolean).join('\n\n')
    const metaBlocks = pages.map(p => pickMeta(p.html))
    const socials = uniq(pages.flatMap(p => discoverSocials(p.html).map(s => JSON.stringify(s)))).map(s => JSON.parse(s))
    const emails = uniq((combined.match(/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}/gi) || []).slice(0, 5))
    const phones = uniq((combined.match(/(\+?\d{1,3}[\s\-]?)?(\(?\d{2,5}\)?[\s\-]?)?[\d\s\-]{5,}/g) || []).slice(0, 5))

    // Prompt: Eine (1) Branche aus DB wählen; Kategorie-Vorschläge >= 25 verlangen.
    const BRANCH_LIST_TEXT = (DB_BRANCHES || []).map(b => `- ${b.name} (slug: ${b.slug})`).join('\n') || '- (keine Branches gefunden)'
...
Gib **exakt** dieses JSON zurück:
{
  ...
  "branch_name": string|null,            // GENAU EINE Branche (Name) aus obiger Liste
  "branch_candidates": [string],         // GENAU 1 Eintrag: identisch zu branch_name
  "category_name": string|null,          // beste primäre Kategorie (frei)
  "category_candidates": [string],       // **mindestens 25** kurze, eindeutige, deduplizierte Einträge
  "categories_extra": [string],

  "services": [ { "name": string, "priority_percent": number|null } ], // 3–8, Summe ≈100 (ganze Zahlen)
...
Emails: ${emails.join(', ') || '–'}
Phones: ${phones.join(', ') || '–'}
Socials: ${socials.map((s:any)=>`${s.kind}:${s.url}`).join(', ') || '–'}

META Titel: ${metaBlocks.map(m => m.meta.title).filter(Boolean).join(' | ') || '–'}
META Desc: ${metaBlocks.map(m => m.meta.description).filter(Boolean).slice(0,2).join(' | ') || '–'}

BESCHREIBUNG:
${freeText}

SEITEN (gekürzt):
${truncate(pages.map(p => `URL: ${p.url}\n${p.text}`).join('\n\n'))}
`.trim()
    ...
    // Fallback: minimaler Heuristik-Output
    if (!extracted) {
      extracted = {
        company_name: null, display_name: null, first_name: null, last_name: null,
        email: emails[0] || null, phone: phones[0] || null, website: seed || null,
        address: { street: null, house_number: null, postal_code: null, city: null, country: null },
        description: freeText || null,
        branch_name: DB_BRANCHES[0]?.name || null,
        branch_candidates: DB_BRANCHES[0]?.name ? [DB_BRANCHES[0].name] : [],
        category_name: null,
        category_candidates: [],
        categories_extra: [],
        services: [],
        links: [...socials, ...(seeds.length ? seeds.map((u: string)=>({ url: u, kind: 'website' })) : [])],
        confidence: 0.4
      }
      modelUsed = 'fallback'
    }

    // Robustheit: Kontakt + Links nachziehen
    ...
    if (!extracted.links || !extracted.links.length) {
      extracted.links = [...socials, ...(seeds.length ? seeds.map((u: string)=>({ url: u, kind: 'website' })) : [])]
    }
    if (Array.isArray(extracted.services)) {
      extracted.services = extracted.services.map((s: any) => ({
        name: String(s?.name || ''),
        priority_percent: Number.isFinite(Number(s?.priority_percent)) ? Math.round(Number(s.priority_percent)) : 0
      }))
    }

    // 🔒 Garantieren: genau EIN Branch-Kandidat = branch_name
    ...
    // 🔹 Kategorien auf **>=25** auffüllen (nur Add, nie löschen)
    const norm = (s:string)=>s?.trim().replace(/\s+/g,' ')||''
    const dedupe = (arr:string[])=>{
      const set = new Set<string>(); const out:string[]=[]
      for (const x of arr) {
        const n = norm(x)
        if (n && !set.has(n.toLowerCase())) { set.add(n.toLowerCase()); out.push(n) }
      }
      return out
    }
    ...
    while (baseCats.length < 25) baseCats.push(`Kategorie ${baseCats.length + 1}`)
