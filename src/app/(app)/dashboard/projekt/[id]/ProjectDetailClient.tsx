...
const Label = ({ children }: { children: React.ReactNode }) =>
  <label className="block text-[13px] font-medium text-slate-700">{children}</label>

const GlassTile = ({
  title, value, Icon,
}: { title: string; value?: string | null; Icon: typeof MapPinIcon }) => (
  <div
    ...
    style={{ backgroundImage: 'radial-gradient(420px 180px at 120% -30%, rgba(15,23,42,0.06), transparent)' }}
  >
...
const StatTile = ({
  title, value, Icon,
}: { title: string; value: string | number; Icon: typeof FolderIcon }) => (
  <div
    className="rounded-2xl border border-white/60 bg-white/70 p-4 shadow-[0_10px_30px_rgba(2,6,23,0.08)] backdrop-blur-xl"
    style={{ backgroundImage: 'radial-gradient(520px 220px at 120% -30%, rgba(15,23,42,0.06), transparent)' }}
  >
...
function OfferChip({
  ...
  return (
    ...
    >
      ...
      <span className={['h-1.5 w-1.5 rounded-full transition', active ? 'bg-white' : 'bg-slate-300 group-hover:bg-slate-400'].join(' ')} />
    </button>
  )
}
...
export default function ProjectDetailClient({ project: initialProject }: { project: Project }) {
  ...
  const [offersMaster, setOffersMaster] = useState<{ id: string; offer_number: string }[]>([])
  ...
  const [roomForm, setRoomForm] = useState<RoomForm>({
    id: null, name:'', width:null, length:null, notes:'', tasks:[], materials:[]
  })
  const [materialsMaster, setMaterialsMaster] = useState<{id:string;name:string;unit:string}[]>([])
  ...
  useEffect(() => {
    supa.from('materials').select('id,name,unit').then(({data}) => setMaterialsMaster((data as any) ?? []))
  }, []) // eslint-disable-line
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setMe(myId)
      try {
        const { data: ownerFlag } = await supa.rpc('is_project_owner', { p_project_id: project.id })
  ...
  useEffect(() => {
    const load = async () => {
      if (!project.customer_id) { setOffersMaster([]); return }
      const { data } = await supa
        .from('offers')
  ...
  const createSigned = async (path: string, expires = 300) => {
    const { data, error } = await supabaseClient().storage.from('projekt').createSignedUrl(path, expires)
  ...
  const prefetchSignedForTab = async (t: TabKey) => {
    ...
    for (const p of missing) {
      try { copy[p] = await createSigned(p, 300) } catch (e) { console.error(e) }
    }
    setSigned(copy)
  }

  useEffect(() => { prefetchSignedForTab(tab) }, [tab, project]) // eslint-disable-line
    ...
    try {
      ...
      if (tab === 'dokumente') {
        ...
        for (const f of Array.from(files)) {
          ...
          const { error } = await supa.storage.from('projekt').upload(key, f, { upsert: false })
          if (error) throw error
          rows.push({ project_id: project.id, path: key, name: f.name, size: f.size })
        }
        const { error: insErr } = await supa.from('project_documents').insert(rows)
        if (insErr) throw insErr
      } else {
        const table = tab === 'vorher' ? 'project_before_images' : 'project_after_images'
        const rows: any[] = []
        for (const f of Array.from(files)) {
          const ext = f.name.split('.').pop() || 'bin'
  ...
  const deleteFileRow = async (
    table: 'project_documents' | 'project_before_images' | 'project_after_images',
    id: string,
    path: string
  ) => {
    // Nur Owner darf löschen (UI ist bereits versteckt, aber hier zusätzliche Absicherung)
    if (!isOwner) { alert('Nur der Inhaber kann Dateien löschen.'); return }
    const { error } = await supa.from(table).delete().eq('id', id)
    if (error) { alert(error.message); return }
    await supa.storage.from('projekt').remove([path]).catch(() => {})
    await refreshProject()
    setSigned(s => { const c = { ...s }; delete c[path]; return c })
  }
  ...
  const saveProject = async () => {
    try {
      ...
      const payload = { title, description, address, object_name: objectName, floor, offer_ids: offerIds }
  ...
  /* ---------- Rooms (Modal-Logik) ---------- */
  const openAddRoom = () => {
    if (!isOwner) { alert('Nur der Inhaber kann Räume hinzufügen.'); return }
    setRoomForm({ id:null, name:'', width:null, length:null, notes:'', tasks:[], materials:[] })
    setRoomOpen(true)
  }
  const openEditRoom = (r: any) => {
    if (!isOwner) { alert('Nur der Inhaber kann Räume bearbeiten.'); return }
    setRoomForm({
      id: r.id,
      name: r.name ?? '',
      width: r.width ?? null,
      ...
      notes: r.notes ?? '',
      tasks: (r.project_room_tasks ?? []).map((t: any) => ({ id: t.id, work: t.work ?? '', description: t.description ?? '' })),
      materials: (r.project_room_materials ?? []).map((m: any) => ({ id: m.id, material_id: m.material_id ?? '', quantity: m.quantity ?? 0, notes: m.notes ?? '' }))
    })
    setRoomOpen(true)
  }
  const saveRoom = async () => {
    ...
    if (!res.ok) {
      ...
      alert(`Raum speichern fehlgeschlagen: ${err?.error ?? res.statusText}`)
      return
    }
    setRoomOpen(false)
    await refreshProject()
  ...
  const deleteRoom = async (roomId: string) => {
    if (!isOwner) { alert('Nur der Inhaber kann Räume löschen.'); return }
    if (!confirm('Diesen Raum wirklich löschen?')) return
    const res = await fetch(`/api/projects/${project.id}/rooms/${roomId}`, { method:'DELETE' })
    if (!res.ok) {
      const err = await res.json().catch(()=>({}))
  ...
  const setRoomField = <K extends keyof RoomForm>(key: K, value: RoomForm[K]) => {
    setRoomForm(prev => ({ ...prev, [key]: value }))
  }

  const addTask = () => {
    setRoomForm(prev => ({ ...prev, tasks: [...prev.tasks, { work: '', description: '' }] }))
  }
  const removeTask = (idx: number) => {
    setRoomForm(prev => ({ ...prev, tasks: prev.tasks.filter((_, i) => i !== idx) }))
  }
  const updateTask = (idx: number, patch: Partial<RoomForm['tasks'][number]>) => {
    setRoomForm(prev => {
  ...
  const addMaterial = () => {
    setRoomForm(prev => ({ ...prev, materials: [...prev.materials, { material_id: '', quantity: 0, notes: '' }] }))
  }
  const removeMaterial = (idx: number) => {
    setRoomForm(prev => ({ ...prev, materials: prev.materials.filter((_, i) => i !== idx) }))
  }
  const updateMaterial = (idx: number, patch: Partial<RoomForm['materials'][number]>) => {
    setRoomForm(prev => {
  ...
  const sendComment = async () => {
    ...
    try {
      ...
      setProject((p: any) => ({ ...p, project_comments: [...(p.project_comments ?? []), optimistic] }))
      setComment('')
      ...
      alert(`Kommentar konnte nicht gesendet werden: ${e.message ?? e}`)
    } finally {
      setSending(false)
    }
  }
  ...
  return (
    <div className="p-6">
      ...
      <div
        ...
        style={{ backgroundImage: 'radial-gradient(1000px 400px at 110% -30%, rgba(15,23,42,0.07), transparent)' }}
      >
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500">
              Erstellt am <span className="tabular-nums font-mono">{new Date(project.created_at).toLocaleDateString('de-DE')}</span>
            </p>
          </div>
        ...
        {roleChecked && !isOwner && (
          <div className="mt-3 rounded-lg border border-slate-200/80 bg-white/80 px-3 py-2 text-xs text-slate-600">
            Du hast Mitarbeiter-Rechte: Du kannst kommentieren sowie Dokumente und Vorher/Nachher-Bilder hochladen. Stammdaten & Räume sind schreibgeschützt.
          </div>
        )}
      </div>
      ...
      <section className="rounded-2xl border border-white/60 bg-white/70 p-5 shadow-[0_10px_40px_rgba(2,6,23,0.08)] backdrop-blur-xl">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-[12px] font-semibold uppercase tracking-wide text-slate-500">Projektdaten</h2>
          {/* Bearbeiten – nur Owner */}
          {!editRequested ? (
            isOwner ? (
              <button className={btnWhite} onClick={() => setEditRequested(true)}>Bearbeiten</button>
            ) : null
          ) : (
            <div className="flex gap-2">
              <button
                ...
                onClick={() => {
                  ...
                  setOfferIds((project.project_offers ?? []).map((x: any) => x.offer_id))
                }}
              >
                Abbrechen
              </button>
        ...
        {!editRequested && (
          <>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
              <GlassTile title="Adresse"  value={project.address}     Icon={MapPinIcon} />
              <GlassTile title="Objekt"   value={project.object_name} Icon={BuildingOfficeIcon} />
              <GlassTile title="Etage"    value={project.floor}       Icon={HashtagIcon} />
              <GlassTile title="Kunde"    value={`${project.customer?.first_name ?? ''} ${project.customer?.last_name ?? ''}`.trim() || '–'} Icon={ClipboardDocumentListIcon} />
              <GlassTile title="Angebote" value={String((project.project_offers ?? []).length)} Icon={CheckCircleIcon} />
              <GlassTile title="Projekt-ID" value={project.id} Icon={HashtagIcon} />
            </div>
            ...
            <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
              <div>
                ...
                <p className="mt-1 whitespace-pre-wrap text-slate-900">{project.description || '–'}</p>
              </div>
              <div>
              ...
              <div className="pt-2">
                    ...
                    .map(o => {
                      ...
                      return (
                        <OfferChip
                          ...
                          onClick={() => {
                            setOfferIds(prev => prev.includes(o.id) ? prev.filter(id => id !== o.id) : [...prev, o.id])
                          }}
                        />
                      )
                    })}
                  {offersMaster.length === 0 && (
                    <span className="text-sm text-slate-500">Keine Angebote (prüfe Kunde)</span>
                  )}
                </div>
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  <button
                    type="button"
                    className={btnWhite}
                    onClick={() => setOfferIds(offersMaster.map(o => o.id))}
      ...
      <section className="mt-6 grid grid-cols-1 gap-3 md:grid-cols-3">
        ...
        <StatTile title="Vorher-Bilder" value={counts.before} Icon={PhotoIcon} />
        <StatTile title="Nachher-Bilder" value={counts.after} Icon={PhotoIcon} />
      </section>
        ...
        <div className="flex flex-wrap items-center gap-2 px-3 pt-3 sm:px-4">
          {(['dokumente','vorher','nachher'] as TabKey[]).map(t => (
            <button
              ...
              title={t}
            >
              {t === 'dokumente' ? 'Dokumente' : t === 'vorher' ? 'Vorher' : 'Nachher'}
            </button>
          ))}
        </div>

        <div className="border-t border-white/60 p-4 sm:p-5">
          <Label>Upload {tab === 'dokumente' ? 'Dokumente' : tab === 'vorher' ? 'Vorher-Bilder' : 'Nachher-Bilder'}</Label>
          <div className="mt-2 rounded-2xl border-2 border-dashed border-slate-300 bg-white/70 p-5 backdrop-blur">
            ...
            />
            {uploading && <div className="mt-2 text-sm text-slate-600">Lade hoch…</div>}
          </div>
          ...
          {tab === 'dokumente' ? (
            (project.project_documents?.length ?? 0) > 0 ? (
              <ul className="mt-4 space-y-2">
                {project.project_documents.map((d: any) => (
                  <li key={d.id} className="flex items-center justify-between rounded-xl border border-white/60 bg-white/80 p-3 shadow-sm backdrop-blur">
                    <button
                    ...
                    <div className="flex items-center gap-3">
                      {d.size ? <span className="text-xs text-slate-500">{Math.round(d.size/1024)} KB</span> : null}
                      {/* Löschen – nur Owner */}
                      {isOwner && (
                        <button
                          onClick={()=>deleteFileRow('project_documents', d.id, d.path)}
            ...
            <>
              <div className="mt-4 grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
                {(tab === 'vorher' ? project.project_before_images : project.project_after_images)?.map((img: any) => {
                  const signedUrl = signed[img.path]
                  return (
                    <div key={img.id} className="group relative overflow-hidden rounded-xl border border-white/60 bg-white/80 shadow-sm backdrop-blur">
                      {signedUrl ? (
                        <img src={signedUrl} alt="" className="h-48 w-full object-cover" />
                      ) : (
                        <div className="grid h-48 w-full place-items-center text-sm text-slate-500">lädt…</div>
                      )}
                      {/* Löschen – nur Owner */}
                      {isOwner && (
                        <div className="absolute right-2 top-2 hidden rounded-lg border border-rose-200 bg-rose-50/95 px-2 py-1 text-xs text-rose-700 shadow-sm group-hover:block">
                          <button onClick={()=>deleteFileRow(tab==='vorher'?'project_before_images':'project_after_images', img.id, img.path)}>Löschen</button>
                        </div>
                      )}
                    </div>
                  )
              ...
              {((tab==='vorher'?project.project_before_images:project.project_after_images)?.length ?? 0) === 0 && (
                <p className="mt-3 text-slate-700">– keine –</p>
              )}
            </>
          )}
        ...
        {project.project_rooms?.length ? (
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            {project.project_rooms.map((room: any) => (
              <div key={room.id} className="space-y-3 rounded-2xl border border-white/60 bg-white/80 p-4 shadow-sm backdrop-blur">
                <div className="flex items-start justify-between">
                  <div>
                    <div className="text-base font-semibold text-slate-900">{room.name}</div>
                    <div className="text-sm text-slate-600">
                      {room.width ? `Breite: ${room.width} m` : ''}{room.width && room.length ? ' • ' : ''}{room.length ? `Länge: ${room.length} m` : ''}
                    </div>
                  </div>
                  {/* Bearbeiten/Löschen – nur Owner */}
                  <div className="flex gap-2">
                    {isOwner && (
                      <button className={btnWhite} onClick={()=>openEditRoom(room)}>Bearbeiten</button>
                    )}
                    {isOwner && (
                      <button
                ...
                {room.notes && <p className="whitespace-pre-wrap text-sm text-slate-800">{room.notes}</p>}

                <div>
                  <div className="mb-1 text-sm font-medium text-slate-700">Arbeiten</div>
                  {room.project_room_tasks?.length ? (
                    <ul className="ml-5 list-disc text-sm text-slate-800">
                      {room.project_room_tasks.map((t: any) => (
                        <li key={t.id}><strong>{t.work}</strong>{t.description ? ` – ${t.description}` : ''}</li>
                      ))}
                    </ul>
                  ) : <div className="text-sm text-slate-500">– keine –</div>}
                </div>

                <div>
                  <div className="mb-1 text-sm font-medium text-slate-700">Materialien</div>
                    ...
                    <div className="overflow-hidden rounded-xl border border-white/60">
                      <table className="w-full text-left text-xs">
                        <thead className="bg-white/80 backdrop-blur">
                          <tr>
                            <th className="p-2 font-semibold text-slate-700">Material</th>
                            <th className="p-2 font-semibold text-slate-700">Menge</th>
                            <th className="p-2 font-semibold text-slate-700">Notiz</th>
                          </tr>
                        </thead>
                        <tbody>
                          {room.project_room_materials.map((pm: any) => (
                            <tr key={pm.id} className="border-t border-slate-200/70">
                              <td className="p-2">{pm.materials?.name ?? '—'}</td>
                              <td className="p-2">{pm.quantity}</td>
                              <td className="p-2">{pm.notes}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
      ...
      <section className="mt-6 rounded-2xl border border-white/60 bg-white/70 p-5 shadow-[0_10px_30px_rgba(2,6,23,0.06)] backdrop-blur-xl">
        <div className="mb-3 flex items-center justify-between">
          <h2 className="inline-flex items-center gap-2 text-sm font-semibold text-slate-800">
            ...
            Kommentare <span className="text-slate-500 font-normal">({counts.comments})</span>
          </h2>
          {counts.comments > 5 && (
        ...
        {lastFive.length === 0 ? (
          ...
          <ul className="space-y-2">
            {lastFive.map((c: any) => (
              <li key={c.id} className="rounded-xl border border-white/60 bg-white/80 p-3 shadow-sm">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="text-xs text-slate-500">
                      {c.user_id === me ? <span className="font-medium text-slate-700">Du</span> : <span className="text-slate-600">Mitarbeiter</span>}
                      <span className="mx-2">·</span>
                      <span title={new Date(c.created_at).toLocaleString('de-DE')}>{timeAgo(c.created_at)}</span>
                    </div>
                    <p className="mt-0.5 whitespace-pre-wrap text-sm text-slate-900">{c.content}</p>
                  </div>
                </div>
              </li>
            ))}
      ...
      {roomOpen && isOwner && (
        <div className="fixed inset-0 z-[200] grid place-items-center bg-slate-900/60 p-3 sm:p-4 backdrop-blur">
          <div className="w-full max-w-[1000px] max-h-[92vh] overflow-hidden rounded-2xl border border-white/60 bg-white/90 shadow-[0_20px_80px_rgba(2,6,23,0.35)] backdrop-blur-xl">
            {/* Header */}
            <div className="sticky top-0 z-10 border-b border-white/60 bg-white/80 px-4 py-4 sm:px-6 backdrop-blur">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-bold text-slate-900">{roomForm.id ? 'Raum bearbeiten' : 'Neuer Raum'}</h3>
                <div className="flex gap-2">
                  <button className={btnWhite} onClick={()=>setRoomOpen(false)}>Abbrechen</button>
                  <button className="rounded-lg bg-slate-900 px-5 py-2 text-sm font-semibold text-white shadow-sm hover:opacity-90" onClick={saveRoom}>
                    Speichern
                  </button>
                </div>
              </div>
            ...
            <div className="max-h-[72vh] overflow-auto px-4 py-4 sm:px-6">
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e)=> {
                      ...
                      setRoomField('width', v === '' ? null : Number.isNaN(Number(v)) ? null : Number(v))
                    }}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e)=> {
                      const v = e.target.value
                      setRoomField('length', v === '' ? null : Number.isNaN(Number(v)) ? null : Number(v))
                    }}
              ...
              <div className="mt-6">
                <div className="mb-2 flex items-center justify-between">
                  <h4 className="text-sm font-semibold text-slate-800">Arbeiten</h4>
                  <button type="button" className={btnWhite} onClick={addTask}>+ Arbeit</button>
                </div>
                {roomForm.tasks.length === 0 ? (
                  <p className="text-sm text-slate-500">Noch keine Arbeiten erfasst.</p>
                ) : (
                  <div className="space-y-3">
                    {roomForm.tasks.map((t, idx) => (
                      <div key={t.id ?? idx} className="rounded-xl border border-white/60 bg-white/80 p-3 shadow-sm">
                        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                          <div className="md:col-span-1">
                            <Label>Arbeit</Label>
                            <input
                              value={t.work}
                              onChange={(e)=>updateTask(idx, { work: e.target.value })}
                        ...
                        <div className="mt-2 flex justify-end">
                          <button type="button" className="rounded-lg border border-rose-200 bg-rose-50 px-3 py-1.5 text-xs text-rose-700 hover:bg-rose-100" onClick={()=>removeTask(idx)}>
                            Entfernen
                          </button>
                        </div>
                      </div>
              ...
              <div className="mt-6">
                <div className="mb-2 flex items-center justify-between">
                  <h4 className="text-sm font-semibold text-slate-800">Materialien</h4>
                  <button type="button" className={btnWhite} onClick={addMaterial}>+ Material</button>
                </div>

                {roomForm.materials.length === 0 ? (
                  <p className="text-sm text-slate-500">Noch keine Materialien erfasst.</p>
                ) : (
                  <div className="overflow-hidden rounded-xl border border-white/60">
                    <table className="w-full text-left text-xs">
                      <thead className="bg-white/80 backdrop-blur">
                      ...
                      <tbody>
                        {roomForm.materials.map((m, idx) => {
                          const selected = materialsMaster.find(mm => mm.id === m.material_id)
                          return (
                            <tr key={m.id ?? idx} className="border-t border-slate-200/70">
                              <td className="p-2">
                                <select
                                  value={m.material_id}
                              ...
                              <td className="p-2">
                                <input
                                  ...
                                  onChange={(e)=> {
                                    const v = e.target.value
                                    updateMaterial(idx, { quantity: v === '' ? 0 : Number.isNaN(Number(v)) ? 0 : Number(v) })
                                  }}
      ...
      {commentsOpen && (
        <div className="fixed inset-0 z-[210] grid place-items-center bg-slate-900/60 p-3 sm:p-4 backdrop-blur">
          <div className="w-full max-w-3xl max-h-[92vh] overflow-hidden rounded-2xl border border-white/60 bg-white/90 shadow-[0_20px_80px_rgba(2,6,23,0.35)] backdrop-blur-xl">
            <div className="sticky top-0 z-10 flex items-center justify-between gap-3 border-b border-white/60 bg-white/80 px-4 py-3">
              <h3 className="text-base font-semibold text-slate-900">Kommentare ({commentsSorted.length})</h3>
              <button className={btnWhite} onClick={()=>setCommentsOpen(false)}>
            ...
            <div className="sticky bottom-0 border-t border-white/60 bg-white/80 px-4 py-3">
              <textarea
                ...
                value={comment}
                onChange={(e)=>setComment(e.target.value)}
                onKeyDown={(e)=>{ if ((e.metaKey || e.ctrlKey) && e.key==='Enter'){ e.preventDefault(); sendComment() } }}
