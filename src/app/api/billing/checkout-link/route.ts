...
async function resolvePriceId(id: string) {
  ...
  throw new Error('STRIPE_PRICE_STARTER must be price_… or prod_…')
}
...
async function handler(req: Request) {
  try {
    ...
    const { data: p, error } = await admin
      ...
      .eq('id', user.id)
      .single()
    if (error || !p) return NextResponse.json({ error: 'Profil nicht gefunden' }, { status: 400 })
