// src/app/(app)/dashboard/buchhaltung/rechnung/rechnung-erstellen/DetailsPositions.tsx
'use client'
...
// lokaler Hilfstyp für strukturierte Adresse
                ...
                {items.map((it) => (
                  <tr key={it.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2 text-right">
                      {Number(it.unit_price).toFixed(2).replace('.', ',')}
                    </td>
                    <td className="px-3 py-2 text-right">
                      <button
                        onClick={() => onPick(it)}
...
export default function DetailsPositions({ onNext }: { onNext: () => void }) {
  ...
  // Nur ein aktiver Droppable: mobil ODER desktop
  ...
  const handleSaveAndNext = async () => {
    ...
    const meta: any = {
      title: title ?? '',
      intro: intro ?? '',
      invoiceNumber: isUpdate ? invoiceNumber : undefined,
      date: date ?? '',
    ...
    if (!isUpdate) meta.idempotencyKey = idemKey
    if (discount) {
      meta.discount = {
    ...
    const payload = { customer: selectedCustomer, positions, meta }
  ...
  const handlePickFromCatalog = (it: CatalogItem) => {
    setPositions(ps => {
      ...
      if (desc) next.push({ type: 'description', description: desc } as any)
      return next
    })
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Kopf-Box: Kunde links, Meta rechts */}
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
        ...
        <div className="mt-6">
          <textarea
            readOnly
            value="Vielen Dank für Ihren Auftrag."
