...
// DEMO-SWITCH: für Screenshots auf true lassen
const DEMO = true
...
export default function DashboardClient({
  ...
  // ---------- DEMO-DATEN ----------
  const demoUserEmail = 'demo@gleno.de'
  ...
  const demoKpis = {
    ...
    invoices: 186,
    revenueYTD: 184523.5,
    revenueLast7: 12340.75,
  }

  const demoSeriesMonths = [
    'Jan',
    'Feb',
    'Mrz',
    'Apr',
  ...
  const demoSeriesCustomers: SeriesPoint[] = demoSeriesMonths.map(
    (m, idx) => ({
      month: m,
      count: [8, 10, 12, 9, 11, 13, 15, 14, 16, 18, 20, 22][idx],
    }),
  )

  const demoSeriesProjects: SeriesPoint[] = demoSeriesMonths.map(
    (m, idx) => ({
      month: m,
      count: [5, 7, 9, 6, 8, 10, 12, 11, 13, 14, 15, 17][idx],
    }),
  )

  const demoSeriesRevenue: RevenuePoint[] = demoSeriesMonths.map(
    (m, idx) => ({
      month: m,
      amount: [
        8000, 9500, 11200, 10500, 12300, 13800, 14500, 15200, 16100, 17500,
        18200, 19600,
      ][idx],
    }),
  )
  ...
  const demoAlerts = {
    ...
    dueFleet: [
      {
        id: 'f1',
        license_plate: 'GF GL 123',
        inspection_due_date: '2025-12-08',
      },
      {
        id: 'f2',
        license_plate: 'GF GL 456',
        inspection_due_date: '2025-12-14',
      },
    ],
    dueTools: [
      {
        id: 't1',
        name: 'Rührwerk RW-500',
        next_inspection_due: '2025-12-03',
      },
      {
        id: 't2',
        name: 'Laser-Messgerät LMG-10',
        next_inspection_due: '2025-12-11',
      },
    ],
  }

  // feste, saubere ISO-Zeiten für heute
  ...
  const demoAppointments = [
    {
      id: 'a1',
      title: 'Objektbesichtigung Musterstraße 12',
      location: 'Musterstraße 12, 12345 Musterstadt',
      start_time: new Date(year, month, day, 9, 0, 0, 0).toISOString(),
      end_time: null,
      reason: 'Projektbesprechung',
    },
    {
      id: 'a2',
      title: 'Planungsmeeting Bad-Sanierung',
      location: 'Büro / Online',
      start_time: new Date(year, month, day, 13, 30, 0, 0).toISOString(),
      end_time: null,
      reason: 'Planung',
    },
  ]
  ...
  // Wenn DEMO true ist, alles überschreiben
  const userEmail = DEMO ? demoUserEmail : realUserEmail
  const kpis = DEMO ? demoKpis : realKpis
  const series = DEMO
    ? {
        customers: demoSeriesCustomers,
        projects: demoSeriesProjects,
        revenue: demoSeriesRevenue,
      }
    : realSeries
  const alerts = DEMO ? demoAlerts : realAlerts
  const appointments = DEMO ? demoAppointments : realAppointments
  const contacts = DEMO ? demoContacts : realContacts
  ...
  const [vaultMetrics, setVaultMetrics] = useState<VaultMetrics>(
    DEMO
      ? {
          contractsMonthly: 1249,
          contractsActive: 18,
          contractsExpiringSoon: 3,
          contractsExpired: 4,
          contractsNextInDays: 12,
          licensesMonthly: 389,
          licensesActive: 9,
          licensesExpiringSoon: 1,
          licensesExpired: 2,
          licensesNextInDays: 7,
          costsIncreaseLast7: 69,
        }
      : {
          contractsMonthly: 0,
          contractsActive: 0,
  ...
  const [vaultLoading, setVaultLoading] = useState(!DEMO)

  // Vault-KPIs nur laden, wenn nicht im DEMO-Modus
  useEffect(() => {
    if (DEMO) return
