...
function drawTableHeader(
  ...
  page.drawLine({ start: { x: M, y: tableY0 }, end: { x: width - M, y: tableY0 }, thickness: 0.5, color: gray })
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: bsData, error: bsErr } = await supabaseAdmin
      ...
      .select('account_holder, iban, bic, billing_phone, billing_email, invoice_prefix, invoice_start, invoice_suffix')
      .eq('user_id', user.id)
      .single()
    ...
    // 5) Rechnungsnummer – Strategie:
    // - Edit: vorhandene Nummer verwenden
    // - Preview: KEINE echte Nummer → "Entwurf"
    // - Commit (Create): echte Nummer via RPC (später, falls nötig)
    let invoiceNumber = existingInvoiceNumber ?? 'Entwurf'
      ...
      if (logoResp.data) {
        ...
        try {
          if (/png/i.test(mime)) logoImage = await pdfDoc.embedPng(arr)
            ...
            try { logoImage = await pdfDoc.embedJpg(arr) } catch { logoImage = await pdfDoc.embedPng(arr) }
          }
    ...
    const addrLine3 = [cPostal, cCity].filter(Boolean).join(' ').replace(/\s+/g, ' ').trim()
    ...
    page.drawText('Rechnungsnr.:', { x: metaX, y: metaY, size: 10, font: fontBold })
    page.drawText(invoiceNumber, { x: metaX + 80, y: metaY, size: 10, font })
    metaY -= 13
    page.drawText('Datum:', { x: metaX, y: metaY, size: 10, font: fontBold })
    page.drawText(toDisplay(baseDateDb), { x: metaX + 80, y: metaY, size: 10, font })
    metaY -= 13
    page.drawText('Zahlen bis:', { x: metaX, y: metaY, size: 10, font: fontBold })
    page.drawText(toDisplay(dueDb), { x: metaX + 80, y: metaY, size: 10, font })
    metaY -= 13
    ...
    if (customerNumber) {
      ...
      page.drawText(customerNumber, { x: metaX + 80, y: metaY, size: 10, font })
      metaY -= 13
    }
...
// Titel
page.drawText(String(title || ''), { x: M, y: y0, size: 14, font: fontBold })
y0 -= titleGap

// Intro-Text (mit Zeilenumbrüchen/Absätzen)
const introText =
  (intro ?? 'Vielen Dank für Ihren Auftrag. Nachfolgend die abgerechneten Positionen:')
    .toString()
    .replace(/\r\n/g, '\n')
...
// 12) Tabelle + Positionen: erst starten, wenn genug Platz; sonst neue Seite
...
if (tableY0 < footerH + minSpaceToFooter + 40) {
  ...
  // Logo auf neuer Seite erneut zeichnen
  ...
  // Tabellenkopf auf neuer Seite etwas tiefer
...
let rowY = tableY0 - initialGapFirst
    ...
    for (let i = 0; i < positions.length; i++) {
      ...
      if (p.type === 'item') {
        ...
        page.drawText(fmtNumber(p.quantity ?? 0), { x: M + 260, y: rowY, size: 10, font })
        page.drawText(p.unit ?? '', { x: M + 320, y: rowY, size: 10, font })
        page.drawText(fmtNumber(p.unitPrice ?? 0), { x: priceX, y: rowY, size: 10, font })
        const totalStr = fmtNumber((p.quantity ?? 0) * (p.unitPrice ?? 0))
        ...
        const sub = positions.slice(0, i).reduce(
          (s, pp) => s + (pp.type === 'item' ? (pp.quantity ?? 0) * (pp.unitPrice ?? 0) : 0),
          0
        )
        const subStr = fmtNumber(sub)
        page.drawText('Zwischensumme:', { x: descX, y: rowY, size: 10, font: fontBold })
        page.drawText(subStr, { x: totalX - fontBold.widthOfTextAtSize(subStr, 10), y: rowY, size: 10, font: fontBold })
        rowY -= lineH + rowSpacing
    ...
    // 13) Summenblock – mit Rabattzeilen
    ...
    if (discount.enabled && discount.value > 0) {
      ...
      const suffix = discount.type === 'percent'
    ...
    for (const pg of pagesAll) {
      ...
      midLines.forEach((ln, i) => {
        ...
        pg.drawText(ln, { x: width / 2 - w / 2, y: footerH - 12 - i * 11, size: 9, font })
      })
    ...
    if (commit) {
      ...
      if (isUpdate) {
        // --- UPDATE: vorhandene Nummer weiterverwenden, idempotency_key NICHT anpacken
        invoiceNumber = existingInvoiceNumber!.trim()
        ...
        const payloadUpdate: Record<string, any> = {
          ...
          date: baseDateDb,         // bleibt durch persistedDateDb stabil, falls vorhanden
          valid_until: dueDb,
          title,
          intro,
          tax_rate: taxRate,
          ...
          pdf_path: filePath,
          due_date: dueDb,
          // idempotency_key ABSICHTLICH NICHT setzen/ändern!
          discount: {
        ...
        // Status nur setzen, wenn NULL
        try {
          await supabaseAdmin
            .from('invoices')
            .update({ status: 'Erstellt', status_changed_at: new Date().toISOString() })
            .eq('user_id', user.id)
        ...
        // --- CREATE: neue Nummer idempotent vergeben
        if (!idempotencyKey || typeof idempotencyKey !== 'string') {
          throw new Error('idempotencyKey fehlt')
        }

        // bereits per idempotencyKey angelegt?
        ...
        if (existing.data?.invoice_number) {
          invoiceNumber = existing.data.invoice_number
        } else {
          // echte Nummer via RPC (atomar)
        ...
        const payloadCreate: Record<string, any> = {
          ...
          pdf_path: filePath,
          due_date: dueDb,
          idempotency_key: idempotencyKey,   // nur beim CREATE schreiben
          discount: {
            enabled: !!discount.enabled,
            label: discount.label ?? 'Rabatt',
            type: discount.type,
    ...
    // Dateiname: Firma bevorzugen, dann Vor-/Nachname; Kundennummer optional
    const safe = (s: string) =>
    ...
    const custNo = safe(((customer as any).customer_number ?? '').toString().trim())
