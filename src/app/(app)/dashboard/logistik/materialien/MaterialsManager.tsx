...
/* ---- Style Shortcuts ---- */
const btnGlass =
  'inline-flex items-center gap-2 rounded-lg border border-white/60 bg-white/80 px-4 py-2 text-sm text-slate-900 shadow-sm backdrop-blur hover:bg-white transition-colors'
const chipGlass =
...
function Tile({
  ...
  return (
    <div
      ...
      style={{
        backgroundImage:
          'radial-gradient(350px 160px at 120% -30%, rgba(15,23,42,0.06), transparent)',
      }}
    >
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center justify-between gap-2 sm:justify-end">
          <span className="hidden items-center rounded-full border border-white/60 bg-white/80 px-2.5 py-1 text-xs font-medium text-slate-700 backdrop-blur sm:inline-flex">
            Materialien: {materials.length}
          </span>
          <button onClick={handleAdd} className={btnGlass}>
      ...
      {filtered.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-white/60 bg-white/60 p-10 text-center backdrop-blur">
          <p className="text-sm text-slate-700">
            Keine Einträge gefunden. Passe die Suche/Kategorie an oder lege
            neues Material an.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
          {filtered.map((m) => {
            const ratio =
              m.critical_quantity > 0
                ? Math.min(m.quantity / m.critical_quantity, 1)
                : 1
            const danger =
              m.critical_quantity > 0 && m.quantity <= m.critical_quantity
                ...
                <div className="relative flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-2">
                      <span className={chipGlass}>
                        <TagIcon className="mr-1 h-3.5 w-3.5" />
                        {m.category || '—'}
                      </span>
                      <span className={chipGlass}>
                        <BeakerIcon className="mr-1 h-3.5 w-3.5" />
                        {m.pattern || '—'}
                      </span>
                    </div>
                  </div>
