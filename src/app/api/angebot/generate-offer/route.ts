...
function drawLines(
  page: PDFPage, lines: string[], x: number, y: number, lineHeight: number, font: PDFFont, size: number
): number {
  let cursorY = y
...
function drawTableHeader(
  ...
  page.drawText(label, { x: totalX - tw, y: tableY0 + 6, size: 10, font: fontBold })
  page.drawLine({ start: { x: M, y: tableY0 }, end: { x: width - M, y: tableY0 }, thickness: 0.5, color: grayLine })
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: bsData, error: bsErr } = await supabaseAdmin
      ...
      .select('account_holder,iban,bic,billing_phone,billing_email,quote_prefix,quote_start,quote_suffix')
    ...
    // --------------------------------------------------------
    let offerNumber: string | undefined = clientOfferNumber?.toString().trim() || undefined
      ...
      if (isUpdate) {
        ...
        if (!offerNumber) {
          ...
          const { error: updErr } = await supabaseAdmin
            ...
            .eq('user_id', user.id)
          if (updErr) throw new Error('Konnte neuen Quote-Startwert nicht speichern')
          offerNumber = `${bsData.quote_prefix ?? ''}${nextQuoteNumber}${bsData.quote_suffix ?? ''}`
        }
    ...
    const { data: { publicUrl } } =
      supabaseAdmin.storage.from('rechnungvorlagen').getPublicUrl(billingSettings.template)
    ...
    if (profile.logo_path) {
      const { data: blob } = await supabaseAdmin.storage.from('logo').download(profile.logo_path)
      if (blob) {
        ...
        try {
          ...
          else { try { logoImage = await pdfDoc.embedJpg(arr) } catch { logoImage = await pdfDoc.embedPng(arr) } }
        } catch { logoImage = await pdfDoc.embedPng(arr) }
        if (logoImage) {
    ...
    if (cStreet || cHausnr) { custY -= 13; page.drawText(`${cStreet} ${cHausnr}`.trim(), { x: M, y: custY, size: 10, font }) }
    if (cPostal || cCity)   { custY -= 13; page.drawText(`${cPostal} ${cCity}`.trim(),   { x: M, y: custY, size: 10, font }) }

    // Meta rechts – inkl. Angebotsnr. (NICHT erhöhen)
    let metaY = baseY
    const metaX = width - M - 150
    page.drawText('Angebotsnr.:', { x: metaX, y: metaY, size: 10, font: fontBold })
    page.drawText(String(offerNumber ?? '—'), { x: metaX + 80, y: metaY, size: 10, font }); metaY -= 13

    page.drawText('Datum:', { x: metaX, y: metaY, size: 10, font: fontBold })
    page.drawText(date ?? '',      { x: metaX + 80, y: metaY, size: 10, font }); metaY -= 13
    page.drawText('Gültig bis:', { x: metaX, y: metaY, size: 10, font: fontBold })
    page.drawText(validUntil ?? '',    { x: metaX + 80, y: metaY, size: 10, font }); metaY -= 13
    const customerNumberDisplay = (customer as any).customer_number ?? '—'
    page.drawText('Kundennr.:', { x: metaX, y: metaY, size: 10, font: fontBold })
    page.drawText(String(customerNumberDisplay), { x: metaX + 80, y: metaY, size: 10, font })
...
if (titleStr.trim()) {
  page.drawText(titleStr, { x: M, y: y0, size: 14, font: fontBold })
  y0 -= 28
}

// Intro: Absätze (\n) + Wortumbruch über die volle Textbreite (linker Rand bis rechter Rand)
const introStr = (intro ?? '').toString().replace(/\r\n/g, '\n')
if (introStr.trim()) {
  const introLineH = lineH // 12
  const introMaxW = width - 2 * M
  const paras = introStr.split('\n').map(s => s.trim()).filter(Boolean)

  let cursorY = y0
  for (let p = 0; p < paras.length; p++) {
    const lines = getWrappedLines(paras[p], introMaxW, font, 10)
    cursorY = drawLines(page, lines, M, cursorY, introLineH, font, 10) - introLineH // kleiner Absatzabstand
  }
...
let rowY = tableY0 - initialGapFirst
    ...
    for (let i = 0; i < positions.length; i++) {
      ...
      if (p.type === 'item' || p.type === 'description') {
        lines = getWrappedLines(p.description || '', (p.type === 'item' ? descW : (width - M - descX)), font, 10)
        const n = Math.max(1, lines.length)
        requiredH = n * lineH + rowSpacing
      }
      if (rowY - requiredH < footerH + minSpaceToFooter) await addNewContentPage()
      ...
      if (p.type === 'item') {
        ...
        const totalStr = fmt((p.quantity ?? 0) * (p.unitPrice ?? 0))
        const twItem   = font.widthOfTextAtSize(totalStr, 10)
        page.drawText(totalStr, { x: totalX - twItem, y: rowY, size: 10, font })
        rowY = endY - lineH - rowSpacing
      } else if (p.type === 'heading') {
        page.drawText(p.description || '', { x: descX, y: rowY, size: 10, font: fontBold })
        ...
        page.drawText('Zwischensumme:', { x: descX, y: rowY, size: 10, font: fontBold })
        page.drawText(subStr, {
          x: totalX - fontBold.widthOfTextAtSize(subStr, 10),
          y: rowY, size: 10, font: fontBold
        })
        rowY -= lineH + rowSpacing
      } else if (p.type === 'separator') {
        page.drawLine({ start: { x: M, y: rowY }, end: { x: width - M, y: rowY }, thickness: 0.5, color: grayLine })
        rowY -= lineH + rowSpacing
      }
    }
    ...
    const sumY = rowY - 50
    page.drawLine({ start: { x: M, y: sumY + 18 }, end: { x: width - M, y: sumY + 18 }, thickness: 0.5, color: grayLine })
    let sy2 = sumY + 2

    page.drawText('Netto', { x: M, y: sy2, size: 10, font })
    page.drawText('EUR',   { x: priceX, y: sy2, size: 10, font })
    { const s = fmt(netSubtotal); page.drawText(s, { x: totalX - font.widthOfTextAtSize(s, 10), y: sy2, size: 10, font }) }
    sy2 -= 16

    if (discount.enabled && discount.value > 0) {
      const label = discount.label?.trim() ? discount.label.trim() : 'Rabatt'
      const suffix = discount.type === 'percent' ? ` (${Number(discount.value).toString().replace('.', ',')}%)` : ''
      const basis  = discount.base === 'net' ? 'auf Netto' : 'auf Brutto'
      page.drawText(`${label} – ${basis}${suffix}`, { x: M, y: sy2, size: 10, font })
      page.drawText('EUR', { x: priceX, y: sy2, size: 10, font })
      { const s = `-${fmt(discountAmount)}`; page.drawText(s, { x: totalX - font.widthOfTextAtSize(s, 10), y: sy2, size: 10, font }) }
      sy2 -= 16

      page.drawText('Netto nach Rabatt', { x: M, y: sy2, size: 10, font })
      page.drawText('EUR',               { x: priceX, y: sy2, size: 10, font })
      { const s = fmt(netAfterDiscount); page.drawText(s, { x: totalX - font.widthOfTextAtSize(s, 10), y: sy2, size: 10, font }) }
      sy2 -= 16
    }

    { // USt
      const taxLabel = `USt (${Number(taxRate ?? 0).toFixed(2).replace('.', ',')} %)`
      page.drawText(taxLabel, { x: M, y: sy2, size: 10, font })
      page.drawText('EUR',     { x: priceX, y: sy2, size: 10, font })
      const s = fmt(taxAmount)
      page.drawText(s, { x: totalX - font.widthOfTextAtSize(s, 10), y: sy2, size: 10, font })
    }
    sy2 -= 16

    page.drawText('Brutto', { x: M, y: sy2, size: 10, font })
    page.drawText('EUR',    { x: priceX, y: sy2, size: 10, font })
    { const s = fmt(grossTotal); page.drawText(s, { x: totalX - font.widthOfTextAtSize(s, 10), y: sy2, size: 10, font }) }
    sy2 -= 16
    ...
    for (const pg of pagesAll) {
      pg.drawLine({ start: { x: M, y: footerH }, end: { x: width - M, y: footerH }, thickness: 0.5, color: grayLine })
      const leftLines = profile.company_name?.trim()
        ? [profile.company_name, `${profile.street} ${profile.house_number}`, `${profile.postal_code} ${profile.city}`]
        : [`${profile.first_name} ${profile.last_name}`, `${profile.street} ${profile.house_number}`, `${profile.postal_code} ${profile.city}`]
      leftLines.forEach((ln, i) => pg.drawText(ln, { x: M, y: footerH - 12 - i * 11, size: 9, font }))
      const midLines = [`Kontoinhaber: ${bsData.account_holder}`, `IBAN: ${bsData.iban}`, `BIC: ${bsData.bic}`]
      midLines.forEach((ln, i) => {
        const w = font.widthOfTextAtSize(ln, 9)
        const centerX = width / 2
        pg.drawText(ln, { x: centerX - w / 2, y: footerH - 12 - i * 11, size: 9, font })
      })
      const rightLines = [`Tel: ${bsData.billing_phone}`, `E-Mail: ${bsData.billing_email}`, profile.website ?? '']
      rightLines.forEach((ln, i) => {
        const w = font.widthOfTextAtSize(ln, 9)
        pg.drawText(ln, { x: width - M - w, y: footerH - 12 - i * 11, size: 9, font })
      })
    }
    ...
    // Upload + Dateiname
    const firstName = ((customer as any).first_name ?? '').toString().trim()
    const lastName  = ((customer as any).last_name ?? '').toString().trim()
    const safeName = `${firstName} ${lastName}`.trim().replace(/[^\w\s\-]/g, '').replace(/\s+/g, '_')
    const custNo = (customer as any).customer_number ? `_${(customer as any).customer_number}` : ''
    ...
    if (commit) {
      ...
      let effectiveOfferId = offerId && String(offerId).length ? String(offerId) : undefined
      ...
      if (effectiveOfferId) {
        const { error: updErr } = await supabaseAdmin
          ...
          .eq('user_id', user.id).eq('id', effectiveOfferId)
        ...
        const { data: ins, error: insErr } = await supabaseAdmin
          .from('offers')
          .insert({ ...payload, status: 'Erstellt', status_changed_at: new Date().toISOString() })
    ...
    return NextResponse.json({ message: err?.message || 'Fehler beim Erstellen/Speichern' }, { status: 500 })
  }
}
